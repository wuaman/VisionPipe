...
void YoloPoseDecoder::decode(const Tensor &output,
  ...
  // batch=1：整段 host 即单帧数据
  decode_frame(host.data(), channels, num_anchors, detections, poses, params,
               letterbox_params, orig_width, orig_height);
}

void YoloPoseDecoder::decode_frame(const float *frame_data, int channels,
                                   int num_anchors,
                                   std::vector<Detection> &detections,
                                   std::vector<PoseResult> &poses,
                                   const YoloPoseParams &params,
  ...
  const int expected = 4 + 1 + params.num_keypoints * 3;
  if (channels != expected || num_anchors <= 0 || frame_data == nullptr) {
    return;
  }
  ...
  for (int i = 0; i < num_anchors; ++i) {
    const float conf = frame_data[4 * num_anchors + i];
    if (conf < params.score_threshold) {
      continue;
    }

    Candidate c;
    const float cx = frame_data[0 * num_anchors + i];
    const float cy = frame_data[1 * num_anchors + i];
    const float w = frame_data[2 * num_anchors + i];
    const float h = frame_data[3 * num_anchors + i];
    c.bbox[0] = cx - 0.5f * w;
    c.bbox[1] = cy - 0.5f * h;
    c.bbox[2] = cx + 0.5f * w;
    c.bbox[3] = cy + 0.5f * h;
    ...
    for (int k = 0; k < params.num_keypoints; ++k) {
      c.kpts[k].x = frame_data[(5 + k * 3 + 0) * num_anchors + i];
      c.kpts[k].y = frame_data[(5 + k * 3 + 1) * num_anchors + i];
      c.kpts[k].score = frame_data[(5 + k * 3 + 2) * num_anchors + i];
    }
  ...
  std::sort(order.begin(), order.end(),
            [&cands](int a, int b) { return cands[a].conf > cands[b].conf; });
  ...
  for (size_t i = 0; i < order.size(); ++i) {
    ...
    for (size_t j = i + 1; j < order.size(); ++j) {
      if (suppressed[order[j]])
        continue;
      if (bbox_iou(cands[order[i]].bbox, cands[order[j]].bbox) >
  ...
  // letterbox 逆映射 → 归一化输出
  for (size_t i = 0; i < order.size() && static_cast<int>(detections.size()) <
    ...
    for (size_t k = 0; k < c.kpts.size(); ++k) {
      const float px =
          (c.kpts[k].x - letterbox_params.pad_x) / letterbox_params.scale;
      const float py =
          (c.kpts[k].y - letterbox_params.pad_y) / letterbox_params.scale;
