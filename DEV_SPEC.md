...
采用 **规范驱动开发（Spec-Driven Development）** 理念：以 DEV_SPEC 为单一事实来源，按规范定义的接口契约和验收标准实现功能，实现后补充测试验证正确性。所有新功能 PR 必须附带对应测试，CI 强制 gate。

对纯逻辑模块（BoundedQueue、DAG 拓扑、配置校验等）可局部采用 TDD 节奏加速实现；对 GPU/硬件依赖模块采用"先实现、后补集成测试"的方式。
