#!/bin/bash
# EfficientNet-B0 ONNX → TensorRT Engine 转换脚本
#
# 用法:
#   ./convert.sh [options]
#
# 选项:
#   --model PATH       ONNX 模型路径（默认: efficientnet_b0.onnx）
#   --output PATH      输出 engine 路径（默认: efficientnet_b0_fp16.engine）
#   --fp16             启用 FP16 精度（默认）
#   --fp32             使用 FP32 精度
...
#
# 示例:
#   ./convert.sh --model efficientnet_b0.onnx --fp16
#   ./convert.sh --model efficientnet_b0.onnx --dynamic-batch --batch-size 8 --max-batch 32
...
if [[ ! -f "$MODEL" ]]; then
    ...
    echo "提示: 可从以下地址下载 EfficientNet ONNX 模型:"
    echo "  https://github.com/onnx/models/tree/main/vision/classification/efficientnet"
    exit 1
fi
...
echo "=========================================="
echo "EfficientNet-B0 ONNX → TensorRT 转换"
echo "=========================================="
echo "输入模型: $MODEL"
echo "输出引擎: $OUTPUT"
echo "精度: $PRECISION"
