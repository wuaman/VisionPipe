#!/bin/bash
# ShuffleNetV2 ONNX → TensorRT Engine 转换脚本
#
# 用法:
#   ./convert.sh [options]
#
# 选项:
#   --model PATH       ONNX 模型路径（默认: shufflenetv2.onnx）
#   --output PATH      输出 engine 路径（默认: shufflenetv2_fp16.engine）
#   --fp16             启用 FP16 精度（默认）
#   --fp32             使用 FP32 精度
...
#
# 示例:
#   ./convert.sh --model shufflenetv2.onnx --fp16
#   ./convert.sh --model shufflenetv2.onnx --dynamic-batch --batch-size 8 --max-batch 32

set -e

# 默认参数
MODEL="shufflenetv2.onnx"
OUTPUT=""
PRECISION="fp16"
BATCH_SIZE=1
MAX_BATCH=32
...
if [[ ! -f "$MODEL" ]]; then
    ...
    echo "提示: 可从以下地址下载 ShuffleNetV2 ONNX 模型:"
    echo "  https://github.com/onnx/models/tree/main/vision/classification/shufflenet"
    exit 1
fi
...
echo "=========================================="
echo "ShuffleNetV2 ONNX → TensorRT 转换"
echo "=========================================="
echo "输入模型: $MODEL"
echo "输出引擎: $OUTPUT"
echo "精度: $PRECISION"
