...
### Step 1.5: 检查 spec review 状态 (Check Review State)

读取 `.claude/spec_review_progress.md`（若存在）：
- 提取各阶段"代码验证结果"中标记为 **不匹配** 的项
- 这些信息用于 Step 2 识别补齐任务、Step 3 确定补齐范围
...
| 类型 | 判定规则 |
|------|----------|
| 补齐任务 | 状态 `[ ]`，且跟踪表备注列有具体缺失说明 |
| 全新任务 | 状态 `[ ]`，备注列为空或仅有依赖信息 |

**查找优先级**：
1. `[~]` 进行中的任务（继续上次未完成的工作）
2. 补齐任务（维护规范一致性，按任务ID升序）
3. 全新任务（按依赖拓扑序）
4. 用户指定任务ID时，直接定位到该任务（忽略优先级）

**依赖判断规则**：
- **补齐任务**：依赖规则放宽。如果其下游任务已 `[x]`，说明核心功能已跑通，补齐任务可独立执行，不受上游状态阻塞。
- **全新任务**：严格依赖。前置任务必须为 `[x]` 才能开始。

**输出格式**：标注任务类型，如：
- `[补齐] T0.2: Frame.user_data 改 map<string, any>`
- `[全新] T5.2: 性能 benchmark + 调优`
...
1. **确定补齐范围**：
   - 从 `spec_review_progress.md` 提取该任务对应阶段的"代码验证结果"中所有 **不匹配** 项
   - 从跟踪表备注列提取缺失项摘要
   - 两者取并集作为待实现清单

2. **读取现有代码**：确认当前实现状态，识别哪些已完成、哪些缺失

3. **只实现缺失部分**：保留并兼容已有功能，不重写已通过测试的代码

4. **清理验证旧行为的测试**：
   - 接口变更（如虚函数签名改变、字段类型变更）会导致旧测试编译失败——这些必须同步修改
   - **语义变更的旧测试更危险**：它们能编译通过，但断言的是被规范否定的旧行为（如 ClassifierNode 结果覆写 `detections[i].class_id` 而非写入 `classifications`）。这类测试必须删除或重写，否则会阻碍新实现或产生误导性的测试通过
   - 判断规则：如果一个测试的断言与新规范的预期行为矛盾，则属于"验证旧行为的测试"，应删除

5. **检查下游适配**：补齐可能涉及接口变更（如类型变化、新增参数），需检查已完成的下游任务是否需要适配。若需要，列出受影响文件并一并修改。
...
**测试场景区分**：
- **全新任务** → 为新功能从头编写测试
- **补齐任务** → 两步走：(1) 先审查已有测试，删除/重写验证旧行为的用例；(2) 再为补齐的新接口编写测试。测试 Agent 的 prompt 中需额外说明接口变更点（旧接口 vs 新接口），避免 Agent 按旧接口写测试
...
**语义级检查**（补齐任务额外步骤）：
- 测试全部通过后，交叉检查：是否有残留的旧测试仍在断言被规范否定的行为？
- 典型信号：测试名称或断言引用了已删除/重命名的接口（如 `infer_frame`、`detections[i].class_id` 被分类覆写、`PipelineBuilder.build()`）
- 若发现，删除或重写这些测试后重新运行
...
3. **更新 spec review 状态**（仅补齐任务）：
   - 若 `.claude/spec_review_progress.md` 存在，将对应的"不匹配"项更新为"已修复 ✓"
   - 这样 vp-spec-review 下次恢复时能看到哪些不匹配项已被修复

4. **Git 提交**：询问用户是否进行 git commit
...
**补齐任务**：
- 依赖规则放宽：补齐任务的下游任务已 `[x]` 说明核心功能已跑通
- 补齐任务可独立执行，不受上游翻回状态阻塞
- 但需注意：补齐涉及的接口变更可能影响下游，实现时需检查适配
